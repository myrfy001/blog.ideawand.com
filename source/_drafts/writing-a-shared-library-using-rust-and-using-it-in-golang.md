...
* 了解一下Golang中Cgo的使用
* 有不少无聊的帖子在争论Golang和Rust哪个更好，通过本次分享，加深对两个语言的了解，不吹不黑。
...
### 一些背景知识
* 我们所开发的满足C-FFI标准的函数库，其参数和返回值都应该尽可能是C语言中的基础数据类型，这也就意味着例如Go中极为常见的Map类型，我们无法通过FFI直接传递。
* C语言的函数返回值，通常也是一个基本数据类型，因此对于Go中多返回值，以及Rust中返回Result、Option之类的操作，无法直接支持
* Rust无法保证FFI边界上的安全，因此我们需要在FFI边界上进行数据的检查，在FFI边界上我们会大量使用unsafe rust。
* 对于上述遇到的各种阻碍，我们需要开发一个中间层来进行适配
...
这个文件中，第一行引入了`stddeh.h`这个文件，这一步主要是为了引入`size_t`这个类型的定义，rust中的`usize`类型对应到C语言中，可以用`size_t`这个类型来表示。《《TODO》《TODO》《TODO》纠正size_t和usize的说法》。
...
* 接下来三行，我们从这三个go变量创建了3个调用C库时需要使用的变量，之所以这样做，是因为`arg1`、`arg2`、`arg3`这三个变量在go中默认都是int类型的，也就是说其所占用的内存空间都是与处理器位宽一致的，而我们在`ffi_demo.h`中定义的三个参数，分别只占用1、2和4个字节，因此我们需要做一下类型的转换，得到了`cArg1`、`cArg2`、`cArg3`这三个变量，其类型分别是`C.uchar`、`C.short`、`C.int`。
...
* 这个函数调用执行完以后，其返回被存到了ret这个变量中，这个变量的类型是`C.ulong`，这也是由于我们在`ffi_demo.h`中定义了函数的返回值类型是C语言中的`size_t`，这个类型对应到cgo中就是`C.ulong`类型，这些对应关系可以通过查阅cgo的文档来获得，不要去背诵这些东西。
* 最后，为了将`C.ulong`与go的内置int类型进行比较，我们需要再做一次类型转换。
...
### 字符串的传递
本小节将提升一些难度，为大家介绍跨语言传递字符串的方法。之所以难度会有提升，是因为相比上一关所有变量都是栈分配而言，本小节的字符串类型涉及到了堆内存的使用。而在使用堆内存的时候，必须处理好谁申请，谁释放的问题。


大家看完上面这一顿操作以后，可能会对FFI接口的效率失去信心……我就是想传递一个字符串，怎么有这么多额外的开销啊……。

对于这个问题，大家先不要慌，上面给大家展示的流程可以说是最安全的一个流程，这些额外开销换来的是绝对的安全，为了优化，我们有两个方向可以尝试：
* 通过人为的约定，减少一些不必要的检查
* 通过调整API接口的设计，避免这些接口出现在Hot-Path上，以减小开销
